		...
		ansiColor('xterm') {
			...
			parallel ArduinoBuilds: {
				lock(quantity: 1, resource: 'arduinoEnv') {
					stage('MySensorsGW (tests,development)') {
						...
						arduino.buildMySensorsGw(config, config.tests, 'Tests-development')
					}
					stage('MySensorsGW (examples,development)') {
						arduino.buildMySensorsGw(config, config.examples, 'Examples-development')
					}
					stage('MySensorsGW (tests,master)') {
						dir(config.library_root) {
							deleteDir()
						}
						...
						config.tests    = findFiles(glob: config.library_root+'tests/**/*.ino')
						config.examples = findFiles(glob: config.library_root+'examples/**/*.ino')
						arduino.buildMySensorsGw(config, config.tests, 'Tests-master')
					}
					stage('MySensorsGW (examples,master)') {
						arduino.buildMySensorsGw(config, config.examples, 'Examples-master')
					}
				}
			}, failFast: true
		}
